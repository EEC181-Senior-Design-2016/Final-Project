...
//****************************************************************************************** readImg
void readImg(void * sdram_ptr, int fileNum){
	FILE *images;
	int fileIndex = 0;
    short rounded = 0;
    short temp = 0;
...
//	images = fopen("testData.txt", "r"); 
    ...
    char fileName[20];
    snprintf(fileName, sizeof(fileName), "Sample/%d.txt", (fileNum + 1));    
	images = fopen(fileName, "r");

    // for 10,000 images, each 784 pixels 784000
	for(fileIndex = 0; fileIndex < 784; fileIndex++){
        ...
        if (counter == 0){
            // put number into sdram 
            ((short*)sdram_ptr)[IMG_BASE2 + (196 * fileNum) +  sdramIndex] = temp; 
...
/*
	...
	fclose(finalLabel);
*/
    finalLabel = fopen("Sample/labels.txt", "r");
	for(index = 0; index < 400; index++){
		fscanf(finalLabel, "%hd", &( ((short*)sdram_ptr)[TESTLABEL_BASE + index] )); 
        //printf("%hd\n", ((short*)sdram_ptr)[TESTLABEL_BASE + index]);
    }
	fclose(finalLabel);
...
int NNalgo(void * sdram_ptr, int * x, void * done_ptr, void * ready_ptr, void * done_ptr2, void * ready_ptr2){
    ...
    lay2calc(sdram_ptr,done_ptr2,ready_ptr2);
